...
public class Resource {
	...
	public static int add(Resource role) {
		...
		if (parentId != null && !"".equals(parentId)) {
			return SimpleDaoTemplate.update(
					"INSERT INTO app_resources(id, name, path, parentId, note) VALUES(?, ?, ?, ?, ?)",
					role.getId(), role.getName(), role.getPath(),
					role.getParentId(),role.getNote());
		}

		return SimpleDaoTemplate.update(
				"INSERT INTO app_resources(id, name, path, note) VALUES(?, ?, ?, ?)",
				role.getId(), role.getName(), role.getPath(), role.getNote());
	}
	...
	public static int edit(Resource resource) {
		...
		if (parentId != null && !"".equals(parentId)) {
			return SimpleDaoTemplate
					.update("UPDATE app_resources SET name = ?, path = ?, parentId = ?, sort = ?, note = ? WHERE id = ?",
							resource.getName(), resource.getPath(), resource.getParentId(), resource.getSort(),
							resource.getNote(),resource.getId());
		}

		return SimpleDaoTemplate.update(
				"UPDATE app_resources SET name = ?, path = ?, sort = ?, note = ? WHERE id = ?",
				resource.getName(), resource.getPath(), resource.getSort(),resource.getNote(), resource.getId());
	}
	...
	public Resource(String id, String name, String path, String parentId,
			String checked, String sort, String note) {
		super();
		this.id = id;
		this.name = name;
		this.path = path;
