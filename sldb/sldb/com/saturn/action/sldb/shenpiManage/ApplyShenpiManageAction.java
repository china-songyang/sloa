...
public class ApplyShenpiManageAction implements IAction {
	@Override
	public IView execute(HttpServletRequest request,
		...
		if ("1".equals(type)) { // 乡镇街-申报
			if(society.getFiles() == null || "".equals(society.getFiles())){
				return new JspView("/app/sldb/lsybjzxt/apply/error.jsp");
			} else{
			...
			if (Society.editToState(society) == 1) {
				...
				personState.setState(society.getState());
				PersonState.add(personState);
				return new JspView("/app/sldb/lsybjzxt/audit/show.jsp");
			} else {
				return new JspErrorView("信息更新失败");
			}
			...
			if (Society.editToState(society) == 1) {
				...
				personState.setState(society.getState());
				PersonState.add(personState);
				return new JspView("/app/sldb/lsybjzxt/check/show.jsp");
			} else {
				return new JspErrorView("信息更新失败");
			}
			...
			if (Society.editToState(society) == 1) {
				...
				personState.setState(society.getState());
				PersonState.add(personState);
				return new JspView("/app/sldb/lsybjzxt/first/show.jsp");
			} else {
				return new JspErrorView("信息更新失败");
			}
			...
			if (Society.editToState(society) == 1) {
				...
				personState.setState(society.getState());
				PersonState.add(personState);
				return new JspView("/app/sldb/lsybjzxt/random/show.jsp");
			} else {
				return new JspErrorView("信息更新失败");
			}
	...
	public String requestMapping() {
		return "/app/sldb/shenpiManage/applyShenpiManage.action";
	}
}
