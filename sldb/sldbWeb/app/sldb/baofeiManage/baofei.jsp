	...
	<script type="text/javascript">
		$(function() {
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/sldb/lsybjzxt/listLsybjzxtByAudit.action',
				queryParams : {
					state : '待审核'
				},
				idField : 'id',//唯一标识列
				...
				toolbar : [{
					...
					handler : function() {
						...
						window.location.href='<%=request.getContextPath()%>/app/sldb/lsybjzxt/query2.action?id='+ rows[0].id +'&view=ok';
						return false;
					}
				}, 	{
					text : '封锁',
					...
					handler : function() {
						...
						baofeiFengsuoVO(ids.join('__'));
					}
				}]
			});
		});

		function baofeiFengsuoVO(id){
			$.messager.confirm('确认封锁', '确认封锁该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/baofeiManage/baofeiFengsuoManage.do?ids=' + id+'&type=1';
				}
			});
			return false;
		}
		...
		function confirmVO(id){
			$.messager.confirm('确认审核', '确认审核该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/shenpiManage/pass.jsp?ids=' + id;
				}
			});
			return false;
		}
		function refuseVO(id){
			$.messager.confirm('确认驳回', '确认驳回该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/shenpiManage/refuse.jsp?ids=' + id+'&type=1';
				}
			});
			return false;
		}
		...
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/sldb/lsybjzxt/query.action?id='+ id;
			return false;
		}
	</script>
</head>
