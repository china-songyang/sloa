	...
	<script type="text/javascript">
		$(function() {
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/sldb/temp/listTempByState.action',
				queryParams : {
					state : '未支付'
				},
				idField : 'id',//唯一标识列
		...
		$(function() {
			$('#queryTable1').datagrid({
				...
				columns : [ [ {
					...
					rowspan : 2,	
					formatter : function(value, rec) {
						return '<span><a href="#" onclick="editVO1(\'' + rec.id + '\');"><img src="<%=request.getContextPath()%>/app/themes/icons/edit.png" width="14" height="14" border="0" /></a>' +
						'&nbsp&nbsp<a href="#" onclick="deleteVO1(\'' + rec.id + '\');"><img src="<%=request.getContextPath()%>/app/themes/icons/cancel.png" width="14" height="14" border="0" /></a>' +
						'&nbsp&nbsp<a href="#" onclick="viewVO1(\'' + rec.id + '\');">预览</a>' +
						'</span>';
					}
				} ] ],
				pagination : true,
		...
		function confirmVO1(id){
			$.messager.confirm('确认审批', '确认审批该选项', function(result){
				if (result){
					...
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/applySociety.action?id='+ id+'&type=7';
				}
			});
			return false;
		}
		
		function refuseVO1(id){
