	...
	<script type="text/javascript">
		...
		function deleteVO(id){
			$.messager.confirm('确认删除项', '确认删除该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/import/sub/delete.action?to=mzzhhd&ids=' + id;
				}
			});
			return false;
		}
		...
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/sldb/import/view.jsp?to=sub/mzzhhd&id='+ id;
			return false;
		}
	</script>
</head>
<body>
	<div id="panel" class="easyui-panel" title="导入数据"
		...
		<form id="queryForm" name="queryForm" method="post" action="<%=request.getContextPath()%>/app/sldb/import/add.action">
			...
			<input id="type" name="type" type="hidden" value="25"></input>
		</form>
	</div>
