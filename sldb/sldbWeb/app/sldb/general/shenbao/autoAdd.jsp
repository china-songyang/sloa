...
<body>
	<form id="addForm" name="addForm" action="<%=request.getContextPath()%>/app/sldb/lsybjzxt/add.action" method="post">
					...
					if(strReadResult == "0")
					{
                      ...
                      /*   document.all['IssuedAt'].value = CVR_IDCard.IssuedAt; //签发机关
                        ...
                        document.all['PictureLen'].value = CVR_IDCard.PictureLen; */ //编码长度
                      	//当前申请人员保障信息统计
                        window.open ('<%=request.getContextPath()%>/app/sldb/lsybjzxt/apply/statistics.jsp?cardNo='+CVR_IDCard.CardNo,'打印页面','height=300,width=300,top=50,left=300,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no');
                   }else{
                        ClearForm();
                        alert(strReadResult);
                   }
