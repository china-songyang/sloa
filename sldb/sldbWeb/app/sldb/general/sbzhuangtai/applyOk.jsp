	...
	<script type="text/javascript">
		$(function() {
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/sldb/lsybjzxt/listLsybjzxtState.action',
				idField : 'id',//唯一标识列
				sortName : 'id',
				sortOrder : 'desc',
				remoteSort : true,
		...
		function viewVO(id){
			window.location.href='<%=request.getContextPath()%>/app/sldb/lsybjzxt/state/showDetail.jsp?pid='+ id+'&type=4';
			return false;
		}
		...
		function confirmVO(id){
			$.messager.confirm('确认审核', '确认审核该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/lsybjzxt/audit/pass.jsp?ids=' + id;
				}
			});
			return false;
		}
		...
		function applyVO(id){
			window.location.href='<%=request.getContextPath()%>/app/sldb/lsybjzxt/applySociety.action?id='+ id+'&type=1';
			return false;
		}
		function endVO(id){
