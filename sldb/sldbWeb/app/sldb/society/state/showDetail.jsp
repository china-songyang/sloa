	...
	<script type="text/javascript">
		$(function() {
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/sldb/society/state/listSocietyDetail.action',
				queryParams : {
		...
		function replayVO(pid){
			$.messager.confirm('确认重申', '确认重申该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/state/replaySociety.action?id='+ pid;
				}
			});
			return false;
		}
