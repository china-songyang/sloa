		...
		$(function() {
			$('#queryTable').datagrid({
				...
				}, --%>
				{
					...
					iconCls : 'icon-add',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/sldb/society/apply/autoAdd.jsp';
						return false;//解决IE6的不跳转的bug
					}
				}, {
					id : 'btnedit',
					...
					handler : function() {
						...
						window.open ('<%=request.getContextPath()%>/app/sldb/society/querySocietyToPrint.action?id='+rows[0].id,'打印页面','height=600,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no');
